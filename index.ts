...
async function sfRequest(method: string, path: string, body?: object) {
  ...
  if (res.status === 401) { await authenticate(); return sfRequest(method, path, body); }
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    switch (name) {
      case "create_record": {
        const result = await sfRequest("POST", `/sobjects/${args!.objectType}`, args!.fields as Record<string,unknown>) as any;
        return { content: [{ type: "text", text: result.success ? `✅ Created ${args!.objectType} — ID: ${result.id}` : `❌ Error: ${JSON.stringify(result)}` }] };
      }
      case "run_soql": {
        const result = await sfRequest("GET", `/query?q=${encodeURIComponent(args!.query as string)}`);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      case "update_record": {
        await sfRequest("PATCH", `/sobjects/${args!.objectType}/${args!.recordId}`, args!.fields as Record<string,unknown>);
        return { content: [{ type: "text", text: `✅ Updated ${args!.objectType} ${args!.recordId}` }] };
      }
      case "get_object_fields": {
        const result = await sfRequest("GET", `/sobjects/${args!.objectType}/describe`) as any;
        const fields = result.fields.map((f: any) => ({ api: f.name, label: f.label, type: f.type }));
